    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg ">
                <div class="p-6">
                    <form method="POST" action="{{ route('edashboard.role.store') }}">
                        @csrf
                        <x-jet-label for="name">Nazwa stanowiska</x-jet-label>
                        <x-jet-input id="name" name="name" type="text" class="text-sm mb-2"></x-jet-input>
                        @error('name')
                        ...
                        <select id="level" name="level" class="form-control mb-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block">
                        ...
                        <x-jet-input id="pay_rate" name="pay_rate" type="number" value="20" class="mb-2     text-sm"></x-jet-input>
                        @error('pay_rate')
                            <p class="text-center text-md text-red-500">Nie ustawiono stawki zarobku na godzinę.</p>
                        @enderror
                        <div class="text-center p-6">
                            <x-jet-secondary-button  type="submit" >Dodaj stanowisko</x-jet-secondary-button>
                        </div>
                    </form>
                </div>
